...
namespace cr {
    ...
    // XOR key for decrypting the token below.
    // (Derived from your encrypted sequence)
    static constexpr std::array<uint8_t, 10> kXorKey = {
        0xb1, 0x9a, 0x94, 0x8d, 0x90,
        0x85, 0xcd, 0xcf, 0xcd, 0xc9
    };

    // Your encrypted token bytes
    ...
    static void toast(std::string const& message, NotificationIcon icon = NotificationIcon::Success) {
        Notification::create("Custom Rates", message, icon)->show();
    }
    ...
    static std::optional<std::string> extractDataString(matjson::Value const& root) {
        auto results = root["results"].asArray();
        if (results.empty()) return std::nullopt;

        auto const& first = results.front();
        if (first["error"].isObject()) return std::nullopt;

        auto rows = first["response"]["result"]["rows"].asArray();
        if (rows.empty()) return std::nullopt;

        auto row = rows.front().asArray();
        if (row.empty()) return std::nullopt;

        if (row.front().isString()) return row.front().asString().unwrapOr("");
        return std::nullopt;
    }
    ...
    static void runSql(
        ...
        req.post(pipelineUrl()).listen([onOk, onErr](web::WebResponse* res) {
            if (!res || !res->ok()) {
                if (onErr) {
    ...
    static std::string sqlRateLevel(int64_t levelID, int blueStars, std::string const& moderator) {
        return "BEGIN; "
    ...
    static void loadAdminsFromDb() {
        // defaults already present; DB can add more
        runSql(
            "SELECT COALESCE(json_group_array(username), '[]') AS data FROM (SELECT username FROM admins ORDER BY username ASC);",
    ...
    static void bootstrap() {
        ...
        // Warm up admin cache from DB (defaults remain as fallback)
        loadAdminsFromDb();
    }
    ...
    static void fetchLevels(
        ...
        runSql(sql, [onOk, onErr](matjson::Value const& root) {
            auto data = extractDataString(root);
            if (!data) {
    ...
    static void fetchRatedMeta(
        ...
        runSql(sqlRatedMeta(levelID), [onOk, onErr](matjson::Value const& root) {
            auto data = extractDataString(root);
            if (!data) {
                onOk(std::nullopt);
                return;
    ...
    static void sendLevelToDb(
        ...
        runSql(sqlSendLevel(levelID, sender), [onDone](matjson::Value const&) {
    ...
    static void deleteSentFromDb(
        ...
        runSql(sqlDeleteSent(levelID), [onDone](matjson::Value const&) {
    ...
    static void rateLevelInDb(
        ...
        runSql(sqlRateLevel(levelID, blueStars, moderator), [onDone](matjson::Value const&) {
    ...
    // ====== POPUP ======
    class CustomRatesPopup : public Popup<> {
    private:
        Tab m_tab = Tab::Sent;
        int m_page = 0;
        int m_requestSeq = 0;
    ...
    protected:
        bool setup() override {
            this->setTitle("Custom Rates");

            auto const size = m_mainLayer->getContentSize();
            auto const centerX = size.width / 2.f;
            ...
            auto makeTabBtn = [&](char const* text, Tab tab, float x) {
                ...
                auto btn = CCMenuItemSpriteExtra::create(
                    spr,
                    this,
                    menu_selector(CustomRatesPopup::onTabPressed)
                );
                btn->setTag(static_cast<int>(tab));
                btn->setPosition({ x, size.height - 24.f });
            ...
            header->setPosition({ centerX, size.height - 46.f });
            m_mainLayer->addChild(header, 5);

            auto column = CCLabelBMFont::create("RANK      LEVEL ID                      META", "bigFont.fnt");
            column->setScale(0.22f);
            column->setColor(ccc3(110, 255, 120));
            column->setPosition({ centerX, size.height - 66.f });
            m_mainLayer->addChild(column, 5);

            m_rowsRoot = CCNode::create();
            m_mainLayer->addChild(m_rowsRoot, 3);
            ...
            m_mainLayer->addChild(navMenu, 5);
            ...
            auto prevBtn = CCMenuItemSpriteExtra::create(
                prevSpr,
                this,
                menu_selector(CustomRatesPopup::onPrevPage)
            );
            ...
            auto nextBtn = CCMenuItemSpriteExtra::create(
                nextSpr,
                this,
                menu_selector(CustomRatesPopup::onNextPage)
            );
            ...
            auto refreshBtn = CCMenuItemSpriteExtra::create(
                refreshSpr,
                this,
                menu_selector(CustomRatesPopup::onRefresh)
            );
            refreshBtn->setPosition({ size.width - 52.f, 18.f });
            ...
            if (isAdmin()) {
                ...
                adminLabel->setPosition({ size.width - 42.f, size.height - 14.f });
                m_mainLayer->addChild(adminLabel, 5);
            }
        ...
        void renderPage() {
            ...
            float const listTop = size.height - 82.f;
            float const listWidth = size.width - 28.f;
            ...
            if (m_pageLabel) {
                m_pageLabel->setString((std::string("Page ") + std::to_string(m_page + 1) + "/" + std::to_string(totalPages)).c_str());
            }
            ...
            if (start >= end) {
                ...
                empty->setPosition({ size.width / 2.f, size.height / 2.f });
            ...
            for (int i = start; i < end; ++i) {
                ...
                auto level = CCLabelBMFont::create((std::string("Level ") + std::to_string(e.levelID)).c_str(), "bigFont.fnt");
                ...
                if (isAdmin() && m_tab == Tab::Sent) {
                    ...
                    auto delBtn = CCMenuItemSpriteExtra::create(
                        delSpr,
                        this,
                        menu_selector(CustomRatesPopup::onDeleteRow)
                    );
        ...
        void onDeleteRow(CCObject* sender) {
            ...
            deleteSentFromDb(levelID, [this](bool ok, std::string const& msg) {
                ...
                if (ok && m_tab == Tab::Sent) loadTab(Tab::Sent);
                this->release();
            });
        }
    ...
    public:
        static CustomRatesPopup* create() {
            ...
            if (ret && ret->initAnchored(470.f, 330.f)) {
                ret->autorelease();
                return ret;
            }
            CC_SAFE_DELETE(ret);
...
class $modify(CustomRatesMenuLayer, MenuLayer) {
    bool init() override {
        ...
        // Start network bootstrap from main menu only.
        cr::bootstrap();
    ...
    void onOpenCustomRates(CCObject*) {
        auto popup = cr::CustomRatesPopup::create();
        if (popup) popup->show();
    }
};

// ====== OPTIONAL SHORTCUT ON LEVEL BROWSER ======
...
class $modify(CustomRatesLevelInfoLayer, LevelInfoLayer) {
    void onEnter() override {
        ...
        auto win = CCDirector::get()->getWinSize();
        btn->setPosition({ 34.f, win.height * 0.5f });
        menu->addChild(btn);
    }
    ...
    void onSendLevel(CCObject*) {
        ...
        // Do not allow re-sending already rated levels
        cr::fetchRatedMeta(levelID, [levelID](std::optional<cr::LevelEntry> rated) {
            if (rated) {
...
class $modify(CustomRatesInfoLayer, InfoLayer) {
    void onEnter() override {
        ...
        cr::fetchRatedMeta(levelID, [label](std::optional<cr::LevelEntry> info) {
            if (!label) return;
